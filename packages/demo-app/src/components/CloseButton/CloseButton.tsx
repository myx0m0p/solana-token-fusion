...
import type { Props } from './CloseButton.types';

const Component: React.FC<Props> = ({ onClick, isDisabled }) => {
  const handleClick = () => {
    !isDisabled && onClick?.();
  };
  ...
  return (
    <button
      type='button'
      className={cn(S.button, { [S.isDisabled]: isDisabled })}
      onClick={handleClick}
      aria-label='close'
    >
      <svg className={S.icon} viewBox='0 0 14 14' fill='#e9cc86' xmlns='http://www.w3.org/2000/svg'>
        <path
          fillRule='evenodd'
          clipRule='evenodd'
